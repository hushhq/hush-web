...
interface UploadContext {
  /** Instance base URL used to resolve relative in-API upload URLs. */
  baseUrl: string
  /** JWT for the in-API upload fallback. Ignored for absolute presigned URLs. */
  token: string
}

/**
 * Performs the direct PUT to whatever URL the presign endpoint returned.
 * URL/auth resolution lives in `attachmentTransport.ts` so the upload and
 * download paths share the same rules — see that module for the URL-shape
 * contract.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    if (resolved.isInApiFallback && ctx.token) {
      try {
        ...
        // setRequestHeader can throw on browsers that have already
        // committed the request — surfaced as a network error below.
      }
    }
