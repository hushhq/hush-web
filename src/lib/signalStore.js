 ...
 * Stores: identity, registrationId, sessions (per remote user+device), preKeys (optional).
 */
...
/**
 * Session key for a remote user+device.
 ...
 */
export async function setSession(db, remoteUserId, remoteDeviceId, stateBytes) {
...
/**
 * @param {IDBDatabase} db
 * @returns {Promise<{ signedPreKey?: number[], signedPreKeySignature?: number[], oneTimePreKeys?: Array<{ keyId: number, publicKey: number[] }> }|null>}
 */
...
/**
 * @param {IDBDatabase} db
 * @param {{ signedPreKey?: number[], signedPreKeySignature?: number[], oneTimePreKeys?: Array<{ keyId: number, publicKey: number[] }> }} blob
 */
